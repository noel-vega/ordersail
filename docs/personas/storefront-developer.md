...
- Not a user in Ordersail's RBAC sense at all — they don't sign in to `merchant-web`. What they hold is credentials the Owner or a permissioned Merchant Staff member (`api_keys:write`, `storefront_origins:write`) hands them: an account-scoped **app key** (`sfk_...`) and a registered **origin**.
...
- Not get silently blocked by CORS in production because a registration step was missed.
...
## The two credentials that gate everything

1. **App key** (`x-app-key`, tenant scoping) — minted in `merchant-web` under Settings → Developer API keys, gated by `api_keys:write`. Identifies *which account's* catalog/cart/checkout this storefront talks to. Set once on `StorefrontClient`'s constructor.
2. **Registered origin** (`storefront_origins` table, CORS allowlist) — scheme+host only (e.g. `https://shop.example.com`), checked against the request's `Origin` header by `storefront-api`'s CORS layer, gated by `storefront_origins:write`. An account can register several (prod, staging, `localhost`). Forget to register the production domain and the storefront works fine locally, then hard-fails with opaque browser CORS errors the moment it's deployed.

On top of both of those, **customer auth** is a third, independent, optional layer — bearer access/refresh tokens from `signUp`/`signIn`, used only for `customer.get`/`customer.update`. Cart and checkout never check it; they run entirely on the `x-cart-token` header, so guest checkout works with zero customer-auth code at all.

## Pain points

- Three separate credential/identity concepts (app key, origin allowlist, customer bearer token) that a developer has to keep straight — mixing up "my request is failing because of CORS" vs. "because of a bad app key" vs. "because the customer session expired" is an easy trap, especially since a CORS failure often just looks like a network error in devtools, not a clear typed `ApiError`.
