// Shared paging primitive for the `GET` list endpoints. DTOs already clamp
// `limit`/`offset` via class-validator, but services call `resolvePageParams`
// too so every list endpoint (products, categories, brands, ...) shapes its
// response the same way, matching apps/merchant-api/src/shared/pagination.
